...
exports.FetchUsersList = (0, catchAsync_1.default)(async (req, res, next) => {
    const users = await user_model_1.User.find({ isDeleted: false }).select("-password -otp -otpExpires -is_two_factor_enabled -isDeleted");
    return (0, AppResponse_1.default)(res, "Users fetched succcessfully.", 200, users);
});
exports.FetchMyDetails = (0, catchAsync_1.default)(async (req, res, next) => {
    const Id = req.user.id;
    const user = await user_model_1.User.findById(Id).select("-password -isDeleted");
    let account;
...
exports.fetchSellerProfile = (0, catchAsync_1.default)(async (req, res, next) => {
    const { sellerId } = req.params;
    const seller = await user_model_1.Seller.findById(sellerId).select("-password -isDeleted -otp -otpExpires -is_two_factor_enabled");
    if (!seller)
...
exports.CreateSellerStore = (0, catchAsync_1.default)(async (req, res, next) => {
    const { name, id } = req.user;
    const { storeName, storeLocation, storeCategory, storeDescription } = req.body;
    const createStore = await user_model_1.Seller.findById(id).select("-password -isEmailVerified -is_two_factor_enabled -otp -otpExpires");
    if (!createStore)
...
exports.editSellerStore = (0, catchAsync_1.default)(async (req, res, next) => {
    const { id } = req.user;
    const { storeName, storeLocation, storeCategory, storeDescription } = req.body;
    const seller = await user_model_1.Seller.findById(id).select("-password");
    if (!seller) {
        return next(new AppError_1.default("Seller not found.", 404));
    }
    if (!seller.store) {
...
exports.ChangeProfileImage = (0, catchAsync_1.default)(async (req, res, next) => {
    if (!req.file)
    ...
    if (prevImage.length > 0) {
        ...
        await (0, uploadAndDeleteImage_1.deleteImage)(key);
    }
    const uploadedImage = await (0, uploadAndDeleteImage_1.uploadMedia)(file);
    const account = await user_model_1.User.findOneAndUpdate({ _id: user.id }, {
        $set: {
...
exports.DeleteUser = (0, catchAsync_1.default)(async (req, res, next) => {
    ...
    const deleteAcc = await user_model_1.User.findById(id).select("-is_two_factor_enabled -password -isEmailVerified -otp -otpExpires");
    if (!deleteAcc)
...
exports.fetchAllStores = (0, catchAsync_1.default)(async (req, res, next) => {
    ...
    const stores = await user_model_1.Seller.find(filterConditions)
    ...
    const totalCount = await user_model_1.Seller.countDocuments(filterConditions);
