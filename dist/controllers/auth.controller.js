...
exports.registerHandler = (0, catchAsync_1.default)(async (req, res, next) => {
    ...
    while (attempts < maxRetries && !emailSent) {
        try {
            await (0, nodemailer_config_1.default)(mailOptions);
            emailSent = true;
        }
        catch (error) {
            attempts++;
...
exports.verifyEmailHandler = (0, catchAsync_1.default)(async (req, res, next) => {
    ...
    const findUser = await user_model_1.User.findOne({ email });
...
exports.loginHandler = (0, catchAsync_1.default)(async (req, res, next) => {
    ...
    await user_model_1.User.findByIdAndUpdate(user._id, { lastLogin: new Date() });
...
exports.logOutHandler = (0, catchAsync_1.default)(async (req, res, next) => {
    //If Web
...
//Completed
exports.refreshAccessTokenHandler = (0, catchAsync_1.default)(async (req, res, next) => {
    ...
    jsonwebtoken_1.default.verify(refreshToken, serviceUrl_1.RefreshToken_Secret_Key, async (err, decoded) => {
        if (err)
            return next(new AppError_1.default("Incorrect or expired refresh token, please log in.", 401));
        const id = decoded.payload.id;
        const findUser = await user_model_1.User.findById(id).select("-password");
        if (!findUser)
...
//Completed
exports.GetTokenDetailsHandler = (0, catchAsync_1.default)(async (req, res, next) => {
...
exports.ChangePasswordHandler = (0, catchAsync_1.default)(async (req, res, next) => {
...
exports.ResetPasswordOtpHandler = (0, catchAsync_1.default)(async (req, res, next) => {
    const { otp, email } = req.body;
    const findUser = await user_model_1.User.findOne({ email });
    const userDate = findUser?.otpExpires;
    const dateToCheck = new Date(userDate);
...
exports.ResetPasswordHandler = (0, catchAsync_1.default)(async (req, res, next) => {
    const { password } = req.body;
    // const id = req.user?.id;
    const id = req.user.id;
    const updatedUser = await user_model_1.User.findByIdAndUpdate(id, { $set: { password: password } }, { new: true });
    if (!updatedUser)
...
exports.GoogleOauthLoginHandler = (0, catchAsync_1.default)(async (req, res, next) => {
    const user = req.user;
    if (user?.status === "fail") {
        return next(new AppError_1.default("This user does not exist", 404));
    }
...
exports.GoogleOauthRegisterHandler = (0, catchAsync_1.default)(async (req, res, next) => {
